...
%% Concatenating relevant values (two datasets : one with trading indicators, one without)
% Display only the first 3 levels
maxLvl = 3;
% Postion of ask volumes in the book for the first 3 levels
askVolPos = 2:4:maxLvl*4;
temporary= [log(book(:,askVolPos));log(book(:,askVolPos+2))];
temporary=reshape(temporary,size(book,1),length(askVolPos).*2);
% log best ask, log best bid, log best ask volume, log best bid volume,
Y_with_dummy=[log(book(:,1)),log(book(:,3)),temporary,buy_initiated,sell_initiated];
Y=[log(book(:,1)),log(book(:,3)), temporary];
timestamps=mess(:,1);
